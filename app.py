...
def load_models():
    ...
    try:
        ...
        # Silently fail to None so app doesn't crash
        tool = None
    
    # 4. Load Sentiment Analyzer
    sid = SentimentIntensityAnalyzer()
    
    return embedder, tool, sid

embedder, tool, sentiment = load_models()
...
class Grader:
    def __init__(self, text, duration=60):
        ...
        self.wpm = (self.n_words / duration) * 60 if duration > 0 else 0
    ...
    def content_score(self):
        ...
        for topic, keys in required.items():
            # Method A: Keyword match (Rule Based)
            if any(k in lower for k in keys):
                hits += 1
                logs.append(f"{topic}: Keyword match")
                continue
            
            # Method B: Semantic match (AI/BERT)
            # "I want to be a pilot" -> Matches "Ambition" context
        ...
        # Scoring: Cap at 30 points (~4.3 pts per topic)
    ...
    def grammar_score(self):
        if tool is None:
            return 8, ["Grammar check unavailable (using default score)"]

        try:
            errs = len(tool.check(self.raw_text))
        except Exception:
            errs = 0
            
        rate = (errs / self.n_words) * 100 if self.n_words else 0
        
        # Formula: 1 - min(errors_per_100 / 10, 1)
...
# Load default text safely
default_val = ""
if os.path.exists("data/sample_text.txt"):
